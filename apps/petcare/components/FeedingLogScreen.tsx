...
function FeedingRow({ feeding }: { feeding: Feeding }) {
  ...
  return (
    <View style={{ backgroundColor: colors.surface, borderRadius: radii.md, padding: spacing(3), marginBottom: spacing(2), borderWidth: 1, borderColor: colors.border }}>
    ...
    <Screen>
      <View style={{ padding: spacing(4), flex: 1 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing(4) }}>
          <Text style={{ fontSize: typography.size.xl, fontWeight: 'bold', color: colors.text }}>Feedings</Text>
          <Pressable
        ...
        {!feedings || feedings.length === 0 ? (
          ...
          <FlatList data={feedings} keyExtractor={(f) => f.id} renderItem={({ item }) => <FeedingRow feeding={item} />} />
        )}
      </View>
    </Screen>
  );
