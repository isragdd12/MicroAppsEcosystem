...
import { Button, ErrorState, Screen, Spinner, Stack, TextInput } from '@microapps/ui';
import { useLocalSearchParams, useRouter } from 'expo-router';
import React, { useEffect, useState } from 'react';
import { Text } from 'react-native';

import { usePet, useUpdatePet } from '../lib/pets';
...
export function EditPetScreen() {
  ...
  async function handleSave() {
    try {
      setSaveError(null);
      await updatePet({ id, patch: { name, species, breed: breed || undefined, birthDate: birthDate || undefined, notes: notes || undefined } });
      router.back();
    } catch (e: unknown) {
  ...
  return (
    <Screen>
      <Stack gap={4} style={{ padding: spacing(4) }}>
        ...
        {saveError && <Text style={{ color: colors.danger, fontSize: typography.size.sm }}>{saveError}</Text>}
        <Button label={isPending ? 'Saving…' : 'Save changes'} onPress={handleSave} />
